...
type LineupTacticsViewProps = {
  formationStorageKey: string;
  fieldPlayers: TacticsFieldPlayer[];
  nextMatch: TacticsNextMatch;
...
export default function LineupTacticsView({
  formationStorageKey,
  fieldPlayers,
  nextMatch,
  viewerPlayerId,
}: LineupTacticsViewProps) {
  ...
  const selectedSlot = selectedPlayer?.lineup_slot ?? null;
  const selectedRole =
    selectedSlot != null
      ? getSlotTactics(formationId, selectedSlot).roleTitle
      : null;
      ...
      <div className="tournament-panel rounded-xl px-3 py-2.5">
        <div>
          ...
          <p className="mt-0.5 text-[9px] text-slate-500">
            Схема {formation.scheme} · можно смотреть задачи всех в основе
          </p>
        </div>
          ...
          <>
            <div className="mt-2 flex gap-1 overflow-x-auto pb-0.5 scrollbar-thin">
              {fieldPlayers.map((player) => {
                ...
                const group = getPositionGroup(slot, player.position);
                const style = getPositionStyle(group);
                const roleShort = getSlotTactics(formationId, slot).roleShort;
