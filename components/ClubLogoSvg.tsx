...
export function clubLogoSizeClass(size: Size = "md"): string {
  return SIZE[size];
}

/** Static SVG crest — safe for server components (fixed gradient ids). */
export default function ClubLogoSvg({
  className = "",
  size = "md",
  idPrefix = "ndfk",
}: ClubLogoSvgProps & { size?: Size }) {
  const bg = `${idPrefix}-bg`;
  const rim = `${idPrefix}-rim`;
  const shine = `${idPrefix}-shine`;
  const ball = `${idPrefix}-ball`;
  const clip = `${idPrefix}-clip`;

  return (
    <div
      className={`club-logo club-logo--stable relative shrink-0 ${SIZE[size]} ${className}`}
      aria-hidden
    >
