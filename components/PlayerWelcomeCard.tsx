  ...
  return (
    <section className="premium-card mb-1.5 overflow-hidden rounded-[14px] sm:mb-6 sm:rounded-[18px]">
      <div className="bg-gradient-to-br from-blue-500/[0.05] via-violet-500/[0.03] to-cyan-500/[0.03] p-1.5 sm:p-5">
        <div
          className={`player-id-strip-home hidden items-center gap-3 sm:flex ${
            hideMobileHeader ? "" : ""
          }`}
        >
          <PlayerAvatar
            ...
            {editingName ? (
              <div className="space-y-2">
                <input
                  ...
                  maxLength={40}
                  autoFocus
                  className="w-full rounded-lg border border-white/15 bg-black/30 px-3 py-1.5 text-base font-bold text-white focus:border-emerald-400/40 focus:outline-none sm:max-w-xs"
                />
              ...
              <>
                ...
                <p className="mt-0.5 truncate text-[11px] text-slate-400">
                  {data.positionGroup}
                  {data.lineupLabel ? ` · ${data.lineupLabel}` : ""}
                  {" · "}
