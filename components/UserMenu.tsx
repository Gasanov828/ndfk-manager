...
export default function UserMenu() {
  ...
  useEffect(() => {
    ...
    const handlePointerOutside = (event: PointerEvent) => {
      ...
      // Desktop dropdown only — mobile sheet handles its own dismiss
  ...
  return (
    <div
      ref={menuRef}
      className={`relative shrink-0 ${open ? "z-[120]" : "z-50"}`}
    >
      <button
        type="button"
        onClick={toggleMenu}
      ...
      <AppBottomSheet
        ...
        panelClassName="border-white/10 bg-slate-950/98"
      >
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 top-[calc(100%+8px)] z-50 hidden w-56 overflow-hidden rounded-2xl border border-white/10 bg-slate-950/98 py-2 shadow-[0_16px_48px_rgba(0,0,0,0.55)] backdrop-blur-xl md:block"
        >
