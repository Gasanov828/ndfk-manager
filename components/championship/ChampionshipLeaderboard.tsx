...
export function ChampionshipMvpBoard({ rows }: { rows: ChampionshipMvpRow[] }) {
  if (rows.length === 0) {
    return (
      <p className="rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-8 text-center text-sm text-slate-500">
        Оценок и MVP сезона пока нет
      </p>
    );
  }
...
export function ChampionshipStatsGrid({
  cards,
}: {
  cards: ChampionshipStatCard[];
}) {
  return (
...
export function ChampionshipAwardsGrid({
  awards,
}: {
